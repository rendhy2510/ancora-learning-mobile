...
const LearningPathPage = () => {
  ...
  const [view, setView] = React.useState<"grid" | "path">("path");
          ...
          <div className="bg-white dark:bg-dark-surface min-h-[calc(100vh-100px)] p-4 lg:p-8">
            <div className="border border-gray-100 dark:border-dark-border rounded-[20px] p-2 lg:p-9 dark:bg-dark-navy/10">
              ...
              >
                ...
                <div className="rounded-[12px] bg-[#F9FAFB]/60 dark:bg-dark-navy/20 p-1.5 mb-4 flex flex-col sm:flex-row items-stretch sm:items-center justify-between overflow-hidden gap-3 sm:gap-0">
                  <div className="flex items-center gap-6 md:gap-10 overflow-x-auto no-scrollbar px-4 py-2 flex-grow min-w-0">
                    {categories.map((cat) => (
                      <button
                        key={cat.name}
                  ...
                  <div className="flex items-center">
                    <div className="hidden sm:block w-[1.5px] h-8 bg-gray-100 dark:bg-dark-border mx-2" />
                    <div className="flex gap-2 px-2 pb-1 sm:pb-0 w-full sm:w-auto">
                      <Button
                        size="sm"
                        onClick={() => setView("grid")}
                        className={`flex-1 sm:flex-none font-medium h-[38px] sm:h-[40px] px-4 sm:px-5 rounded-[10px] whitespace-nowrap transition-all duration-300 text-[12px] sm:text-[14px] ${
                          view === "grid"
